...
namespace CRUDGenerator.Templates.Scripts.MySQL
{
...
" + primaryKey + @"" + indexes + @"" + uniqueKeys + @"" + foreignKeys + @"";
            return res;
        }
        ...
        private static string getColumnScript(Column column)
        {
            ...
            switch (column.Type)
            {
                case CRUDGenerator.Database.Type.Int:
                    res = "`" + column.Name + "` int(" + column.Length + ") " + isNull;
                    break;
                case CRUDGenerator.Database.Type.Int_AI:
                    res = "`" + column.Name + "` int(" + column.Length + ") AUTO_INCREMENT " + isNull;
                    break;
                case CRUDGenerator.Database.Type.Boolean:
                    res = "[" + column.Name + "[ BIT(1) " + isNull;
                    break;
                case CRUDGenerator.Database.Type.String:
                    res = "[" + column.Name + "] VARCHAR(" + column.Length + ") " + isNull;
                    break;
                case CRUDGenerator.Database.Type.Text:
                    res = "[" + column.Name + "] TEXT " + isNull;
                    break;
