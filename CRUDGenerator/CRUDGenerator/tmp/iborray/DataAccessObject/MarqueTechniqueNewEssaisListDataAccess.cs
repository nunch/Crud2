...
{
    public class MarqueTechniqueNewEssaisListDataAccess
    {
        
        public MessageModel GetAllMarqueTechniqueNewEssaisList()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<MarqueTechniqueNewEssaisList> list = (from tmp in db.MarqueTechniqueNewEssaisList select tmp).ToList();
