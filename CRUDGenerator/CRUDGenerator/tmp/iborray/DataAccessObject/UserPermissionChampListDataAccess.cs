...
{
    public class UserPermissionChampListDataAccess
    {
        
        public MessageModel GetAllUserPermissionChampList()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<UserPermissionChampList> list = (from tmp in db.UserPermissionChampList select tmp).ToList();
