...
{
    public class AdminEssaisListDataAccess
    {
        
        public MessageModel GetAllAdminEssaisList()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<AdminEssaisList> list = (from tmp in db.AdminEssaisList select tmp).ToList();
