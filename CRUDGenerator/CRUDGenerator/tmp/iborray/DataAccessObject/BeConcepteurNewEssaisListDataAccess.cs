...
{
    public class BeConcepteurNewEssaisListDataAccess
    {
        
        public MessageModel GetAllBeConcepteurNewEssaisList()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<BeConcepteurNewEssaisList> list = (from tmp in db.BeConcepteurNewEssaisList select tmp).ToList();
