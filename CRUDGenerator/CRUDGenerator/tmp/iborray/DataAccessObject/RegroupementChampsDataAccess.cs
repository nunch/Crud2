...
{
    public class RegroupementChampsDataAccess
    {
        
        public MessageModel GetAllRegroupementChamps()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<RegroupementChamps> list = (from tmp in db.RegroupementChamps select tmp).ToList();
