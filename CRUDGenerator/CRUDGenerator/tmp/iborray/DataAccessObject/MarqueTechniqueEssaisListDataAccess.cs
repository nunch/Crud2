...
{
    public class MarqueTechniqueEssaisListDataAccess
    {
        
        public MessageModel GetAllMarqueTechniqueEssaisList()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<MarqueTechniqueEssaisList> list = (from tmp in db.MarqueTechniqueEssaisList select tmp).ToList();
