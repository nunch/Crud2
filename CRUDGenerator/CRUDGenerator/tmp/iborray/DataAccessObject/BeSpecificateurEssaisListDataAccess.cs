...
{
    public class BeSpecificateurEssaisListDataAccess
    {
        
        public MessageModel GetAllBeSpecificateurEssaisList()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<BeSpecificateurEssaisList> list = (from tmp in db.BeSpecificateurEssaisList select tmp).ToList();
