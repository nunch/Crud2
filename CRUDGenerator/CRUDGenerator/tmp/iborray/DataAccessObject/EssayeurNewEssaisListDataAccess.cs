...
{
    public class EssayeurNewEssaisListDataAccess
    {
        
        public MessageModel GetAllEssayeurNewEssaisList()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<EssayeurNewEssaisList> list = (from tmp in db.EssayeurNewEssaisList select tmp).ToList();
