...
{
    public class ProgrammeViewDataAccess
    {
        
        public MessageModel GetAllProgrammeView()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<ProgrammeView> list = (from tmp in db.ProgrammeView where tmp.IsSuppr == false select tmp).ToList();
