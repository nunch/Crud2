...
{
    public class ChampValuesViewDataAccess
    {
        
        public MessageModel GetAllChampValuesView()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<ChampValuesView> list = (from tmp in db.ChampValuesView select tmp).ToList();
