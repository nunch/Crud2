...
{
    public class SupplyChainEssaisListDataAccess
    {
        
        public MessageModel GetAllSupplyChainEssaisList()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<SupplyChainEssaisList> list = (from tmp in db.SupplyChainEssaisList select tmp).ToList();
