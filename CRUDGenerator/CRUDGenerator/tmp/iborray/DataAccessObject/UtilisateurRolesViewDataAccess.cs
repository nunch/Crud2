...
{
    public class UtilisateurRolesViewDataAccess
    {
        
        public MessageModel GetAllUtilisateurRolesView()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<UtilisateurRolesView> list = (from tmp in db.UtilisateurRolesView select tmp).ToList();
