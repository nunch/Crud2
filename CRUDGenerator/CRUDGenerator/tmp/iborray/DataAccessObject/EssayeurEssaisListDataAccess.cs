...
{
    public class EssayeurEssaisListDataAccess
    {
        
        public MessageModel GetAllEssayeurEssaisList()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<EssayeurEssaisList> list = (from tmp in db.EssayeurEssaisList select tmp).ToList();
