...
{
    public class SupplyChainNewEssaisListDataAccess
    {
        
        public MessageModel GetAllSupplyChainNewEssaisList()
        {
            try
            {
                using (var db = new DataAccessContainer())
                {
                    List<SupplyChainNewEssaisList> list = (from tmp in db.SupplyChainNewEssaisList select tmp).ToList();
